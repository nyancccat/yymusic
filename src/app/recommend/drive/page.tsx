...
export default function RecommendDrivePage() {
    ...
    async function loadOnce() {
      const listsByPlatform = await Promise.all(
        PLATFORMS.map(async (platform) => {
          ...
          const preferred = lists.find((list) => /热歌/.test(list.name)) || lists[0];
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <SongList songs={songs} showAlbum={false} showDuration={false} />
      )}
    </div>
  );
}
